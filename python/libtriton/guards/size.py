...
class SizeGuard(CheckGuard):
    _regex_pattern: re.Pattern = re.compile(
        rf"{CheckGuard._regex_variable}\.size\(\)\[{CheckGuard._regex_int}\] == {CheckGuard._regex_int}"
    )
    ...
    def build_ir(
        ...
        *,
        ...
        size: ir.Value = dlpack.shape(i64_type, dl_tensor, dim, loc=loc)
        expected: ir.Value = arith.constant(i64_type, self.expected, loc=loc)
        return arith.cmpi(arith.CmpIPredicate.eq, size, expected, loc=loc)
