...
func.func @normalize_launch_from_builtin_operand(%arg0: tensor<4xf32>, %arg1: tensor<4xf32>, %n: i32) {
      ...
      torch_ext.triton_kernel_launch @kernel blocks in(%c1, %c1, %c1) threads in(%c1, %c1, %c1) dynamic_shared_memory_size %c0 args(%vt0, %arg1, %n : !torch.vtensor<[4], f32>, tensor<4xf32>, i32) : index
      return
}

// CHECK-LABEL: func.func @normalize_launch_from_builtin_operand
// CHECK: torch_ext.triton_kernel_launch @kernel blocks in(%{{.*}}, %{{.*}},
...
func.func @normalize_launch_from_to_chain(%arg0: !torch.vtensor<[4], f32>, %n: i32) {
      ...
      %c1 = arith.constant 1 : index
      torch_ext.triton_kernel_launch @kernel blocks in(%c1, %c1, %c1) threads in(%c1, %c1, %c1) args(%vt0, %n : !torch.vtensor<[4], f32>, i32) : index
      return
}
...
// CHECK: torch_ext.triton_kernel_launch @kernel blocks in(%{{.*}}, %{{.*}}, %{{.*}}) threads in(%{{.*}}, %{{.*}}, %{{.*}}) args(%[[T0]], %{{.*}} : tensor<4xf32>, i32) : index
...
func.func @preserve_non_from_builtin_operand(%arg0: !torch.vtensor<[4], f32>, %n: i32) {
      %c1 = arith.constant 1 : index
      torch_ext.triton_kernel_launch @kernel blocks in(%c1, %c1, %c1) threads in(%c1, %c1, %c1) args(%arg0, %n : !torch.vtensor<[4], f32>, i32) : index
      return
}

// CHECK-LABEL: func.func @preserve_non_from_builtin_operand
// CHECK: torch_ext.triton_kernel_launch @kernel blocks in(%{{.*}}, %{{.*}}, %{{.*}}) threads in(%{{.*}}, %{{.*}}, %{{.*}}) args(%arg0, %{{.*}} : !torch.vtensor<[4],f32>, i32) : index