...
class LibTritonGraphModule(object):
    ...
    def compile(self, *args: Any, **kwargs: Any) -> Any:
        ret, function = Function.build(self.fn, self.ctx)(*args, **kwargs)
    ...
    def _emit_unbox_any_arg(
        ...
        if isinstance(target_ty, ir.MemRefType):
    ...
    def _emit_dispatch_call(
        ...
        callee_ty: ir.FunctionType = ir.FunctionType(func_op.type)
        assert len(callee_ty.inputs) == 4 and len(callee_ty.results) == 1, (
            f"submodule wrapper signature mismatch: {func_op.sym_name.value}: {callee_ty}"
        )
        call_op: func.CallOp = func.CallOp(
            [self._i32_type],
            func_op.sym_name.value,
    ...
    def _emit_dispatch_if_chain(
        self,
        dispatch_iter: Iterator[Function],
        packed_values_ptr: ir.Value,
        packed_args_ptr: ir.Value,
        num_args: ir.Value,
        packed_result_ptr: ir.Value,
        ...
        item: Optional[Function] = next(dispatch_iter, None)
        if item is None:
    ...
    def _build_execution_engine_callable(
        ...
        def f(*args: Any) -> Any:
            result = fn(*args)
            if isinstance(result, (list, tuple, tvm_ffi.Array)):
    ...
    def _build_builtin_pipeline(chip: str) -> str:
        passes: List[str] = [
            ...
            "finalize-tvm-ffi-call",
            f"gpu-lower-to-nvvm-pipeline{{index-bitwidth=64 cubin-chip={chip} cubin-format=fatbin opt-level=3}}",
            "lower-affine",
